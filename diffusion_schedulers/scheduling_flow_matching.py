...
class FlowMatchEulerDiscreteSchedulerOutput(BaseOutput):
    ...
    Args:
        prev_sample (`torch.FlowTensor` of shape `(batch_size, num_channels, height, width)` for images):
            Computed smaple `(x_{t-1})` of previous timestep. `prev_sample` should be used as next model input in the 
            denosing loop 

    
    """

    prev_sanple: torch.FloatTensor




class PyramidFlowMatchEulerDiscreateScheduler(SchedulerMixin, ConfigMixin):

    """ 
    Euler scheduler. 

    This model inherits from [`SchedulerMixin`] and [`ConfigMixin`]. Check the superclass documentation for the generic 
    methods the library implements for all schedulers such as leading and saving.

    Args:
        num_train_timesteps (`init`, defaults to 1000):
            The number of diffusion steps to train the model.

        timestep_spacing (`str`, defaults to "linspace"):
            The way the timestep should be scaled. Refer to Table 2 of the [Common Diffusion Noise Schedulers and 
            Sample Steps are Flowed](https://huggingface.co/papers/2305.08891) for more information.
    ...
    def init_sigmas_for_each_stage(self):
        ...
        for i_s in range(stages):
            ...
            end_indice = int(start_indice[i_s + 1] * training_steps)
        ...
        for i_s in range(stages):
            ...
            self.timestep_per_stage[i_s] = torch.from_numpy(timestep[:-1])
    ...
    def begin_index(self):
        """ 
        The index for the first timestep. It should be set from pipline with `set_begin_index` method.
        """
    ...
    def set_begin_index(self, begin_index: int = 0):
        ...
        """
        Sets the begin index for the scheduler. This function should be run from pipline before the inference.
    ...
    def set_timesteps(self,
        ...
        stage_timesteps = self.timestep_per_stage[stage_index]
        ...
        sigmas = torch.from_numpy(ratios).to(device)
    ...
    def index_for_timestep(self,
                           timestep,
                           schedule_timestep=None):
        ...
        # This way we can ensure we don't accidently skip a sigma in 
        # case we start in the middle of the denoiding schedule (e.g. for image-to-image)
    ...
    def step(
        ...
        process from the learned model outputs (most often the predict noise).
        ...
        Args:
            ...
            timestep (`float`):
                The current discreate timestep in the diffusion chain.
        ...
        Returns:
            [`~schedulers.scheduling_euler_discrete.EulerDiscreteSchedulerOutput`] or `tuple`:
                ...
                return, otherwise is tuple is returned where the first element is the sample tensor.
        ...
        if (
            ...
            raise ValueError(
                (
                    ...
                    " `EulerDiscreateScheduler.step()` is not supporte. Make sure to pass "
        ...
        return FlowMatchEulerDiscreteSchedulerOutput(prev_sanple=prev_sample)
