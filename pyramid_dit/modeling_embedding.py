...
def get_1d_sincos_pos_embed_from_grid(embed_dim, pos):
    ...
    if embed_dim % 2 != 0:
        raise ValueError("embed_dim must be disible by 2")
    ...
    pos = pos.reshape(-1)  # (M,)
    out = np.einsum("m, d -> md", pos, omega)   # (M, D/2), other product 
...
def get_timestep_embedding(
    ...
    """ 
    This matches the implmentation in Denoising Diffusion Probabilistic Models: Create sinusoidal timestep embeddings.
    :param timestep: a 1-D Tensor of N indices, one per batch element. These may be fractional.
    :param embedding_dim: the dimension of the output. :param max_perios: controls the minimum frequency of the 
    ...
    # concat since and cosine embeddings 
...
class CombineTimestepConditionEmbeddings(nn.Module):
    ...
    def __init__(self,
        ...
        self.time_proj = Timestep(num_channels=256, flip_sin_to_cos=True, downscale_freq_shift=0)
    ...
    def forward(self, timestep, pooled_projection):
        ...
        timestep_emb = self.text_embedder(timestep_proj.to(dtype=pooled_projection.dtype))
...
class CombineTimestepEmbeddings(nn.Module):
...
class PatchEmbed3D(nn.Module):
    ...
    def __init__(
        ...
        self.bias_size = height // patch_size
        ...
        if pos_embed_type is None:
            ...
            pos_embed = get_2d_sincos_pos_embed(
                embed_dim,
                grid_size,
                base_size=self.bias_size,
                interpolation_scale=self.interpolation_scale
            )


            persistant = True if pos_embed_max_size else False 
            self.register_buffer("pos_embed", torch.from_numpy(pos_embed).float().unsqueeze(0), persistent=persistant)
    ...
    def cropped_pos_embed(self, height, width, ori_height, ori_width):
        ...
        if self.pos_embed_max_size is None:
            raise ValueError("`pos_embed_max_size` must be set from cropping.")
        ...
        if self.interp_condition_pos:
            ...
            if ori_height != height or ori_width != width:
                ...
                spatial_pos_embed = torch.nn.functional.interpolate(spatial_pos_embed, size=(height, width), mode='billinear')
    ...
    def forward_func(self, 
        ...
        if self.pos_embed_type == 'sincos':
            ...
            # Spatial positional embeding, Interpolate or crop positonal embeddings as needed 

            if self.pos_embed_max_size:
                pos_embed = self.cropped_pos_embed(height, width, ori_height, ori_width)

            else:
                raise ValueError("Not implementated sincos pos embed without sd3 max pos crop")
    ...
    def forward(self, latent):
        ...
        Arguments: 
            ...
            flatten_input (bool): True indicates flatten the latent into 1D sequence
